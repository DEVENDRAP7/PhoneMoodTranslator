...
public class CalendarAdapter extends RecyclerView.Adapter<CalendarAdapter.CalendarViewHolder> {
    ...
    public CalendarAdapter(List<DailyStats> monthlyStats, int daysInMonth, int firstDayOfWeek) {
    ...
    public void onBindViewHolder(@NonNull CalendarViewHolder holder, int position) {
        ...
        if (dayOfMonth <= 0 || dayOfMonth > daysInMonth) {
            ...
            if (statsForDay != null) {
                // Fixed: cardDayRoot and tvDate are now accessible via the holder
                holder.cardDayRoot.setCardBackgroundColor(Color.parseColor("#FFF8E1"));
                holder.cardDayRoot.setStrokeColor(Color.parseColor("#FFE082"));
                ...
                holder.petView.setMoodData((int)(statsForDay.totalUsageTime / 60000));
            } else {
                holder.cardDayRoot.setCardBackgroundColor(Color.WHITE);
                holder.cardDayRoot.setStrokeColor(Color.parseColor("#F5F5F5"));
    ...
    private DailyStats findStatsForDay(int day) {
        for (DailyStats s : monthlyStats) {
            try {
                if (Integer.parseInt(s.date.split(" ")[0]) == day) return s;
            } catch (Exception e) {
                return null;
            }
        }
    ...
    static class CalendarViewHolder extends RecyclerView.ViewHolder {
        TextView tvDate;
        MoodPetView petView;
        MaterialCardView cardDayRoot; // Added Missing Variable

        public CalendarViewHolder(@NonNull View itemView) {
            super(itemView);
            // Changed: Finding the views by the IDs in your XML
            tvDate = itemView.findViewById(R.id.tvDateNumber);
            petView = itemView.findViewById(R.id.calendarPet);
            cardDayRoot = itemView.findViewById(R.id.cardDayRoot); // Added Initialization
        }
    }
}